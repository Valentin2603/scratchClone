...
function createWorkspaceBlock(type) {
    ...
    if (type === 'new_value') {
        ...
        newBlock.innerHTML = '<input class="block-input" placeholder="переменная или arr[0]"> = <input class="block-input" placeholder="значение">';
    }
    else if (type === 'create_array') {
        newBlock.innerHTML = 'array <input class="block-input" placeholder="имя массива"> = [<input class="block-input" placeholder="1, 2, x + 3">]';
    }
    else if (type === 'IF') {
        newBlock.innerHTML = `
            <div class="if-header">
                IF <input class="block-input" placeholder="условие (пример: arr[0] > x)">
            </div>
...
class RNP {
    ...
    static tokenize(exp) {
        ...
        while (i < source.length) {
            ...
            const arrayAccess = rest.match(/^([A-Za-zА-Яа-яЁё_][A-Za-zА-Яа-яЁё_0-9]*)\[(\d+)\]/);
            if (arrayAccess) {
                tokens.push(arrayAccess[0]);
                i += arrayAccess[0].length;
                continue;
            }
            ...
            throw new Error('Некорректный символ: ' + char);
        }
